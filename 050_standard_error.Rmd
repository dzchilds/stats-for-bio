...
```{r, echo=FALSE}
load(file = "./_object_cache/plant_morphs.rda")
```

In the previous chapter we introduced the idea that whenever we construct a point estimate of a population parameter, it will be imperfect, in the sense that it won't exactly reflect the true value of the parameter. This uncertainty is always present, which meana a point estimate is effectively useless on it's own. Fortunately, we can use the machinery of statistics to help us quantify the uncertainty, and once we have done that, we can start to provide meaningful answers to our scientific questions. We *will* get to 'the answer' step soon. But to do this, we're have to first develop the uncertainty idea a bit more.

## Sampling error

Let's carry on with the purple morph / green morph example introduced in the last chapter (skim back over the example if you can't remember it). We were trying to evaluate whether the purple morph frequency was greater than 25% in our new population. We took one sample of 

What happens if we repeat the random sampling process, leading to a new, completely independent sample? Here is the second sample from the population:
...
Once we know how to calculate the sampling distribution for a particular problem, we can start to make statements about sampling error (to quantify uncertainty), and we can begin to make meaningful comparisons to address scientific questions. The good news is that we don't have to work any of this out for ourselves---statisticians have already done this for many different situations.

### The effect of sample size

Arguably the most important aspect of any sampling scheme is the **sample size**: the number of observations (objects or items) in a sample. To see how sample size influences the sampling distribution, and to understand why it matters, let's carry on with the simulation example. We'll repeat the multiple sampling exercise, but this time we'll do it twice. First we'll take a sample of `r sampsize2` individuals each time, and then we'll take a sample of `r sampsize3` individuals on each occasion. We'll take a total 100000 samples in both cases:
