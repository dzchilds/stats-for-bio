...
We can develop our simple simulation example a bit more to explore the consequences of sampling error. However, rather than taking one sample at a time, we'll use R to simulate thousands of independent samples. The number of plants sampled ('n') will always be `r sampsize1`. Here's the important bit: every sample is drawn from the same population, i.e. the population parameter (purple morph frequency) will never change across samples. This means any variation we observe will be due to nothing more sampling error.
...
The short answer to this question is that before we can start to use frequentist statistics we need to have a sense of...
...
Once we understand these links, we're able to start investigating how frequentist statistics works. That's what we'll do in the next block of work... 
