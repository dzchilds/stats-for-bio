...
Populations are conceptualised as fixed but unknown quantities within the framework of frequentist statistics. The goal of an analysis is to learn something about one or more populations by collecting data---'the population' is defined by the investigator, and the 'something' could be anything we know how to measure. For example, a social scientist might be interested in understanding the political attitudes of university students, a climate change scientist might want to know how much carbon is stored in UK peatland areas, and a behavioural ecologist might want to understand how much time individuals spend foraging for food and searching for mates. 

The obvious question is, how do we learn about a population?
...
Let's assume that we've also been studying a neighbouring population that exhibits the same polymorphism. Our studies shave shown that...

*   The colour polymorphism in the sister population is controlled by a single gene with two alleles: a recessive mutant allele ('P') confers the purple colour, and the dominant wild-type allele ('G') makes plants green. Moreover, population genetic studies have shown that the two alleles are present in a ratio of 1:1. 

*   There seems to be no observable fitness difference between the two morphs in the sister population. What's more, about 25% of plants are purple in the population we've been studying, i.e. the alleles seem to be in Hardy-Weinberg equilibrium, further indicating that there is no selection operating.

We've noticed that the purple morph seems to be more common in the new study population. Our hypothesis is that the is at a selective advantage in this population. The corresponding prediction is that the frequency of the purple morph will be greater than 25% in the new study population.

(This isn't the strongest test of our hypothesis by the way. Really, we need to study both allele and genotype frequencies. Sadly, the government has pulled all research funding for plant polymorphisms since Brexit happens, so this is the best we can do)
...
save(plantdata, sampsize1, sampsize2, sampsize3, 
     file = "./_object_cache/plant_morphs.rda")
```
