...
1. Go to MOLE, dowload the STORMS.CSV file, and place it somewhere 'sensible' (i.e. make sure it is somewhere permanent, which you can find again).
...
- If you don't set the working directory, R will do it for you. This is probably not the folder you want to use.
...
rm(list = ls())
```
Can you remember what this does? Why is it a good idea always include it?
</div>
...
What happened? Look at the 'Environment' tab in the top right pane. Can you see something called 'storms'? What information does `str` give you?
...
The second view: __Data frames are table-like objects__. Take another look at our simple data frame. This clearly has rows and columns. 
...
Er, what the @^$£ is a 'tibble'?... They were called a 'tbl' last year (pronounced 'table'). Basically, a special version of the data frame. **dplyr** tends to produce them. We can convert if we need to, e.g.
...
Trying using `str` and `glimpse` on `storms`. Then try `View`...
...
<div>
<br>
**It is useful to classify the verbs according to what they work on:__
...
<div id="box-text">
Use the `filter` function on the `storms` dataset to select the rows containing just the observations associated with 'Hurricanes' that took place in 1997 or later. Just print the result to the Console.
...
<div id="box-text">
Use the `select` function on the `storms` dataset to make a new data set containing only `name`, `year` and `pressure`. Assign a name to the result (i.e. use `<-`) and then check that it contains the right variables using the `glimpse` function.
</div>
</div>
...
<div id="box-text">
The sea level pressure of every observation is measured in millibar. Use the `mutate` function on the `storms` dataset to make a new variable (inside `storms`) that is the sea level pressure in Pascal. Call the new variable `pressure_Pa`.
...
The **dplyr** package inmports a special operator, called 'the pipe'. It looks like this: `%>%`. This allows us to chain together a sequence of operations, reading from left to right and top to bottom. For example, take a look at this sequence of calculations:
...
- Although `summarise` looks a lot like `mutate`, it is designed to construct a completely new, much smaller, dataset containing summaries of one or more variables.
- You may use more than one `SummaryStat = <expression>` at a time (seperated by `,`) if you need to construct several summaries.
</div>
...
<div id="box-text">
You're going to use `group_by` and `summarise` together, along with the pipe (` %>% `) in this exercise. Use the `group_by` and `summarise` functions on the `storms` dataset to calculate the minimum and maximum wind speed associated with each month. Call the summary variables `min_speed` and `max_speed`.
...
__Quick remoind of how all this works__:
...
<div id="box-text">
Repeat th at last example (a scatter plot of wind speed vs pressure), but now try to customise the point colours, size, and transparency.
...
__Customising your plot__
<div id="box-text">
Repeat th at last example (a scatter plot of wind speed vs pressure), but now make the point colour depend upon storm type (i.e. the `type` variable).
</div>
</div>
...
We use histograms and dot plots to understand the __distribution__ of a variable. They summarise the number of observations occuring in a contiguous series of bins. Use `geom_histogram` to construct a histogram and `geom_dotplot`. Here is an example:
