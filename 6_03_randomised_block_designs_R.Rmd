...
Notice that we only specified the two main effects (species and blocks)---we cannot test for the interaction. In general an interaction can be included using a colon (the `:` symbol) with the two interacting variables either side of it as follows: 

```{r, eval = FALSE}
Midge ~ Species + Block + Species:Block
```
...
```{r, echo=FALSE}
invisible(sapply(damsels.model[1:3], function(line) cat(line, "\n")))
```
...
```{r, echo=FALSE}
invisible(sapply(damsels.model[4:8], function(line) cat(line, "\n")))
```
