...
The data are stored in a CSV file SLUGIT.CSV. The rate of movement is stored in `Distance` variable and the treatment codes are in the `SlugitTreat` variable (levels: full dose, half dose, one tenth dose, control).

Read in the data and carry out an ANOVA on the movement rates following different doses of Slugit. Explore the data visually --- using a box and whiskers plot or a multi-panel dot plot --- and then fit a one-way ANOVA model. Use the plot and fitted model to evaluate the assumptions of the model:
