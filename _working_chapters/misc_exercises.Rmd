...
You should work through the exercises step-by-step, following the instructions carefully. At various points we will interupt the flow of instructions with a question. Make a note of your answer so that you can complete the associated MOLE quiz, which is called 't-tests'.   
...
The paired *t*-test is a very useful technique because it can significantly improve the power of an experiment. You can get an idea of the value of the paired-sample test by seeing what would have happened if we had ignored the pairing structure of glycolip data introduced in the [Paired-sample *t*-tests] chapter.

and analysed it with an unpaired, two-sample approach (N.B.---This analysis is wrong!):
```{r}
...
Use an appropriate *t*-test to decide whether there is a significant difference between the amount of infection of bean roots in the two different soils.
