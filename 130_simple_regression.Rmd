...
The simple statistical tools we have encountered so far have been concerned with comparing the mean(s) of a numeric variable (e.g. plant weight, skull size, glucosamine concentrations) in some way. This can be done either with respect to an expected value (i.e. one-sample *t*-test), or among two groups (i.e. paired-sample *t*-test and two-sample *t*-test). In the latter case we are interested in how the mean of the numeric variable differs among two different groups defined by the values of a categorical variable. The categorical variable often encode information about different experimental treatments, but they could also represent something that we have measured in a non-experimental, observational study (e.g. habitat type, high vs. low elevation). 

(If we needed to, we could evaluate differences in means of more than two categories by using a related, but more sophisticated, tool called Analysis of Variance (ANOVA). We'll get to this later in the course.) 

One way to think about these kinds of tools and is to conceptualise them as considering a relationship between between a numeric and categorical variable. Specifically, they ask how the mean of the numeric variable changes among the categories. This chapter is about a different kind of question. What should we do when two numeric variables have been measured **on the same objects/individuals** in a study? For example, we could sample fish and measure their length and weight, we could surveu grassland plots and measure soil pH and species diversity, or we migt manipulate temperature and measure enzyme activity in bed bugs.

In each of these settings we want to understand the relationship between the two different numeric variables. We may be interested in knowing:
...
Clearly it would be useful to have a measure of how likely it is that the relationships we think we see could have arisen as a result of sampling variation. In addition to judging the significance of a relationship, we may also be interested in describing the relationship mathematically – i.e. finding the equation of the best fitting line through the data. Regression techniques allow you to do this. In fact, using regression, the relationship on the left would be judged significant, the one on the right would not. 
## Understanding regression: an example {#understand-regression}
...
You should begin working through the *Vicia* germination example from this point. To do this, you will need to download the GLUCOSE.CSV file from MOLE, placing it in your working directory. 
```

Once you have downloaded the data it into an R data frame, giving it the name `vicia.germ`:
```{r, echo=FALSE}
vicia.germ <- read.csv(file = "./data_csv/GLUCOSE.CSV")
```
As always, make sure you use `View`, `glimpse`, etc to examine the data before you proceed.

First of all, as with most analyses, we should visuaise the data set. We just need to produce a simple scatter plot, which is easy to do with `ggplot2`:
```{r, fig.width=4}
...
A final comment. It is really important that you understand what a residual from a regression represents. Residuals pop up a lot when evaluating statistical models (not just regression). For example, the assumptions of simple linear regression models are typically evaluated in terms of the properties of their residuals. We're going to learn how to do this in the next chapter. For thsi reason, you should ask a TA for help if you are still not sure what a residual is.
...
OK, let's return to our example and see how to carry out a simple regresion in R.
