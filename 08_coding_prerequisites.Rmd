...
<div class="well">
Don't worry if you are an Environmental Sciences student joining us from Geography. We will run extra catch-up sessions for the first three weeks to help you learn the basic R skills needed to do this course, though we won't cover everything the APS students did last year. The material in this section won't make much sense at the moment, but it will after the first three weeks (we promise).
</div>

#### Using packages

Remember, R packages extend the basic functionality of R so that you can do more with it. A package bundles together R code, data, and documentation in a way that is easy to use and share with other users. Last year we learned how to use some of the functions provided by the `dplyr` package (for data manipulation) and the `ggplot2` package (for plotting), and we applied these to datasets bundled up in the `nasaweather` package. We are occasionally going to use the `dplyr` and `ggplot2` packages again this year, so you need to remember how to use R's package system in order to access these.

You can revise how to use the package system in the [packages]({{site.baseurl-L1}}/help-packages.html) topic. It isn't difficult to use, and we will obviously help you if you run into difficulties. Remember, installing a package, and then loading and attaching the package, are different activities. You only have to install it once, but you have to load the package each time you want to use it in a new R session (i.e. every time you start RStudio).
