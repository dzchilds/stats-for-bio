...
We can start to tackle this question by setting up an appropriate null hypothesis---the null hypothesis is that there is actually no difference between the population means. We then work out what the sampling distribution of the differences between sample means looks like under this null hypothesis (yes, this is the 'null distribution'). The key assumption that makes these calculations 'doable' is that the variable is **normally distributed** in each population. If this assumption is valid, then the null distribution will have a known form. We can then use this knowledge to carry out a test of statistical significance.

So how is this different from the permuation test? The logic is virtually identical in fact. The only difference is that because we're prepared to make the normality assumption, we only need to use a few peices of information to construct out the significance test: the sample sizes of A and B, the sample variances of A and B, and the estimated difference between the sample means. No complicated resampling of data is involved. 
...
Degrees of freedom are closely related to the idea of sample size. In a nutshell, the greater the degrees of freedom associated with a test, the more likely it is to detect an effect if it's present (it has more 'statistical power'). We 'lose' degrees of freedom whenever we have to estimate a quantity like a mean. That's a bit of an over simplication, but it's enough to be getting on with.
