...
The data seems roughly normal, so let's carry out a one-sample *t*-test on the calculated differences. This is test is easy to do in R:
```{r}
t.test(glycolipid_diffs$Difference)
```
We don't have to set the `data` argument to carry out a one-sample *t*-test on the differences. We just passed along the numeric vector of differences extracted from `glycolipid_diffs` (using the `$` operator). What happened to the `mu` argument used to set up the null hypothesis? Remember, the null hypothesis is one where the population mean is zero. R assumes that this is 0 if we don't supply it, so no need to set it here.
